...
describe('StoreFrontService', function() {
  ...
  describe('->generateICA', function() {
    describe('when storefront replies with all good things', function() {
    ...
    describe('when storefront replies with a 200, but JSON instead of an ICA file', function() {
      beforeEach('generate-ica', function(done) {
        this.loadMainPage = this.storeFrontServer.get('/Citrix/StoreWeb')
          .set('Accept', 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8')
          .set('Upgrade-Insecure-Requests', '1')
        ...
        this.getICAFileContents = this.storeFrontServer.get('/Citrix/StoreWeb/some-launch-id.ica')
          ...
          .reply(200, { fileFetchUrl: null, pollTimeout: 5, status: "Retry" })

        this.sut.generateICA((error) => {
          this.error = error
          done()
        })
      })
      ...
      it('should yield an error', function () {
        ...
        expect(this.error.message).to.contain('Unexpected JSON response instead of ICA File: ')
      })
    })
  })
})
