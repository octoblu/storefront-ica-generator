...
class StoreFrontICAGenerator {
  constructor({ storeFrontUrl, query, domain, username, password }) {
    ...
    debug(JSON.stringify({ storeFrontUrl, query, domain, username, password }, null, 2))
  ...
  explicitLogin(callback) {
    ...
    debug('explicitLogin', JSON.stringify(options, null, 2))
    this.request.post(options, (error, response) => {
      if (error) return callback(error)
      debug('explicitLogin result', response.statusCode, JSON.stringify(response.body, null, 2))
  ...
  explicitLoginAttempt(callback) {
    ...
    debug('explicitLoginAttempt', JSON.stringify(options, null, 2))
    this.request.post(options, (error, response) => {
      if (error) return callback(error)
      debug('explicitLoginAttempt result', response.statusCode, JSON.stringify(response.body, null, 2), JSON.stringify(response.headers, null, 2))
      debug('explicitLoginAttempt cookies', this.getCookies())
      if (response.statusCode > 399) {
        return callback(new Error(`Unexpected status cod (${response.statusCode}) when ExplicitAuth/LoginAttempt`))
      }
      callback()
    })
  }
  ...
  generateICA(callback) {
    this.getResource((error, resource) => {
      ...
      this.getICAFileContents(resource, (error, icaContents) => {
        ...
        debug('modified ica contents', JSON.stringify(icaContents, null, 2))
  ...
  getAuthMethods(callback) {
    ...
    debug('getAuthMethods', JSON.stringify(options, null, 2))
    this.request.post(options, (error, response) => {
      if (error) return callback(error)
      debug('getAuthMethods result', response.statusCode, JSON.stringify(response.body, null, 2))
      if (response.statusCode > 399) {
  ...
  getICAFileContents(resource, callback) {
    debug('getICAFileContents resource', JSON.stringify(resource, null, 2))
    ...
    debug('getICAFileContents', JSON.stringify(options, null, 2))
    this.request.get(options, (error, response, contents) => {
      if (error) return callback(error)
      debug('getICAFileContents result', response.statusCode, JSON.stringify(response.body, null, 2))
      if (response.statusCode > 399) {
        return callback(new Error(`Unexpected status code (${response.statusCode}) when fetching ICA File`))
      }
      if (isJSON(contents)) {
        return callback(new Error(`Unexpected JSON response instead of ICA File: ${contents}`))
      }
  ...
  getResources(callback) {
    ...
    series({
      ...
      if (error) return callback(error)
      debug('getResources result', JSON.stringify(result, null, 2))
  ...
  homeConfiguration(callback) {
    ...
    debug('homeConfiguration', JSON.stringify(options, null, 2))
    this.request.post(options, (error, response) => {
      if (error) return callback(error)
  ...
  listResources(callback) {
    ...
    debug('listResources', JSON.stringify(options, null, 2))
  ...
  loadMainPage(callback) {
    ...
    debug('loadMainPage', JSON.stringify(options, null, 2))
