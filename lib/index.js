...
class StoreFrontICAGenerator {
  ...
  generateAndLaunch(callback) {
    this.generateICAFile((error, icaFilePath) => {
      ...
      opn(icaFilePath).then(() => {
        ...
        callback(new Error('Unable to launch ica file'))
      })
    })
  }
  ...
  generateICA(callback) {
    this.getResource((error, resource) => {
      ...
      this.getICAFileContents(resource, (error, icaContents) => {
        ...
        callback(null, this.modifyICA(icaContents))
      })
    })
  }
  ...
  generateICAFile(callback) {
    tmp.tmpName({ postfix: ".ica" }, (error, icaFilePath) => {
      ...
      this.getResource((error, resource) => {
        ...
        this.writeICAFileContents(resource, icaFilePath, callback)
      })
    })
  }
  ...
  modifyICA(icaContents) {
    ...
    const twiSettings = ['TWIMode=On','TWIFullScreenMode=1', 'TWIIgnoreWorkArea=1', 'TWISeamlessFlag=1']
    const replaceTWIMode = replace(/TWIMode=Off/, multiline(twiSettings))
    return replaceTWIMode(icaContents)
  }
  ...
  writeICAFileContents(resource, icaFilePath, callback) {
    this.getICAFileContents(resource, (error, icaContents) => {
      if (error) return callback(error)
